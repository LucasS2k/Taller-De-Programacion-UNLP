...
function InformarExistenciaNombreSocio (a: arbol; N:cadena15):boolean;
begin
...
InformarExistenciaNombreSocio := InformarExistenciaNombreSocio(a^.HD, N) 
or InformarExistenciaNombreSocio (a^.HI, N);
end;
end;
...
function EdadPromedio (a: arbol; CantidadSocios: integer): real;
 procedure CalcularEdades (a:arbol; var sumatotal: integer);
 begin
  if (a <>  nil) then begin
   sumatotal := sumatotal + a^.dato.edad;
   CalcularEdades (a^.HI, sumatotal);
   CalcularEdades (a^.HD, sumatotal);
 end;
 end;
 var 
 SumaTotal: integer;
 begin
 sumatotal:= 0;
 CalcularEdades(a, sumatotal);
 EdadPromedio := sumatotal / CantidadSocios;
end;
var a: arbol; 
var N: cadena15;
var CantidadSocios:integer;
Begin
  randomize;
  GenerarArbol (a);
  ...
  writeln ('Ingrese un nombre para buscar: '); readln (N);
  if (InformarExistenciaNombreSocio (a, N)) then 
   writeln (N, ' encontrado ')
   else writeln ( N,' no encontrado');
  CantidadSocios := InformarCantidadSocios(a);
  writeln('La cantidad total de socios es: ', CantidadSocios);
  writeln (EdadPromedio (a, CantidadSocios):2:2);
